...
const Desktop: React.FC = () => {
  ...
  // Futuristic gradient backgrounds with animated mesh
  const darkBg = 'radial-gradient(circle at 20% 30%, rgba(88, 28, 135, 0.4) 0%, transparent 50%), radial-gradient(circle at 80% 70%, rgba(59, 130, 246, 0.35) 0%, transparent 50%), radial-gradient(circle at 50% 50%, rgba(16, 185, 129, 0.2) 0%, transparent 70%), linear-gradient(135deg, #0a0a1f 0%, #1a0d2e 25%, #0f0920 50%, #1a1042 75%, #0d0820 100%)';
...
const DesktopIcon: React.FC<{ appId: string; title: string }> = ({ appId, title }) => {
  ...
  return (
    ...
    >
      ...
      >
        <span className="relative z-10 drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">{meta.glyph}</span>
        {/* Glass reflection */}
        <span
